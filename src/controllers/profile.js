...
export const getCurrentProfile = async ({ user }, res) => {
  try {
...
// Gets Profile by userId
export const getById = async ({ params: { userId } }, res) => {
...
export const addExperience = async (
  { body: { title, company, location, from, to, current, description }, user },
  res
) => {
  // Validate Inputs
...
export const delExperience = async ({ params: { expId }, user }, res) => {
  // Check if user_id is a valid Object ID
  if (!expId.match(/^[a-fA-F0-9]{24}$/)) {
    return errorRespone('invalid id', 'The provided id is not a valid one', res)
  }
  // Find Proile By ID
  try {
...
export const delEducation = async ({ params: { eduId }, user }, res) => {
  // Check if user_id is a valid Object ID
  if (!eduId.match(/^[a-fA-F0-9]{24}$/)) {
    return errorRespone('invalid id', 'The provided id is not a valid one', res)
  }
  // Find Proile By ID
  try {
